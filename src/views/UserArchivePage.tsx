...
import { fileUrl, formatBytes, fetchUserArchives, type SanityArchive } from "@/lib/sanity";
...
const UserArchivePage = () => {
  const { data: items = [], isLoading, error } = useQuery({
    queryKey: ["sanity-user-archives"],
    queryFn: fetchUserArchives,
    staleTime: 1000 * 60,
  });
    ...
    <div className="min-h-screen bg-muted">
      ...
      <main className="container py-8 md:py-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">👥 이용자 자료실</h1>
            <p className="text-sm text-muted-foreground mt-1">이용자를 위한 안내 자료와 양식을 확인하세요.</p>
          </div>
        </div>
        ...
        <div className="bg-card rounded-2xl shadow-sm overflow-hidden">
          {isLoading ? (
            ...
            <>
              {/* Desktop table */}
              <table className="hidden md:table w-full" aria-label="이용자 자료실 목록">
                    ...
                    return (
                      <tr key={it._id} className="hover:bg-accent/30">
                        <td className="px-5 py-4">
                          <Link to={`/user-archive/${it._id}`} className="font-medium text-foreground hover:text-primary transition-colors">
                            {it.title}
                          </Link>
                          {it.description && (
                ...
                {items.map((it) => {
                  ...
                  return (
                    <li key={it._id} className="p-4 space-y-2">
                      <Link to={`/user-archive/${it._id}`} className="block font-semibold text-base hover:text-primary">
                        {it.title}
                      </Link>
                      ...
                      {url ? (
                        ...
                        <Link to={`/user-archive/${it._id}`} className="block">
...
export default UserArchivePage;
