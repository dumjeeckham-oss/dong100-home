...
const ArchivePage = () => {
  ...
  const getDownloadUrl = (item: SanityArchive) =>
    item.file?.asset?.url || fileUrl(item.file?.asset?._ref);
                  ...
                  return (
                    <li key={it._id} className="p-4 space-y-2">
                      ...
                      <div className="text-xs text-muted-foreground flex flex-wrap gap-x-3 gap-y-1">
                        ...
                        <span className="inline-flex items-center gap-1"><Calendar size={12} />{new Date(it.publishedAt).toLocaleDateString("ko-KR")}</span>
                      </div>
                      {url ? (
